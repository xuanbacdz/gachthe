...
function autocard365($loaithe, $pin, $seri, $menhgia, $code){  
    global $CMSNT;
    $response = curl_get("https://autocard365.com/api/card-auto.php?type=$loaithe&menhgia=$menhgia&seri=$seri&pin=$pin&APIKey=".$CMSNT->site('api_autocard365')."&callback=".BASE_URL('callback.php')."&content=$code");
...
function trumthe($loaithe, $pin, $seri, $menhgia, $code){
    global $CMSNT;
    $url = 'https://gachthe1s.com/chargingws/v2?sign='.md5($CMSNT->site('partner_key').$pin.$seri).'&telco='.$loaithe.'&code='.$pin.'&serial='.$seri.'&amount='.$menhgia.'&request_id='.$code.'&partner_id='.$CMSNT->site('partner_id').'&command=charging';
    $ch = curl_init();
